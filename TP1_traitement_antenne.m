...
v = sqrt(sigma_v/2)*(randn(M,1)+i*randn(M,1)); % [5x1]      % bruit pour chaque capteur
s = sqrt(sigma_v/2)*(randn(K,1)+i*randn(K,1));               % [1x1]      % signal recu
A=zeros(M,1);                 % [5x1]      % direction arrivee de la source determinee par chaque capteur

for i =1:M
    A(i,1) = exp(-i*pi*(i-1)*sin(theta0));
end
...
% N=500 echantillons
V_n = sqrt(sigma_v/2)*(randn(M,N)+i*randn(M,N));% [5x500]    % bruit pour chaque capteur, pour chaque échantillon
S_n = sqrt(sigma_s/2)*(randn(K,N)++i*randn(K,N));              % [1x500]    % signal recu de la source, pour chaque echantillon
A=zeros(5,1);                  % [5x1]      % direction arrivee de la source determinee par chaque capteur pour un echantillon
A_n = zeros(5,N);              % [5x500]    % % direction arrivee de la source determinee par chaque capteur pour chaque echantillon
...
subplot(5,1,1);plot(xx,Y_n(1,:));title('s1');
subplot(5,1,2);plot(xx,Y_n(2,:));title('s2');
subplot(5,1,3);plot(xx,Y_n(3,:));title('s3');
subplot(5,1,4);plot(xx,Y_n(4,:));title('s4');
subplot(5,1,5);plot(xx,Y_n(5,:));title('s5');
...
for teta=-pi/2:pi/180:pi/2 % on parcourt tous les angles
    ...
    for m = 1:M % w pour chaque capteur
        a_teta_fixe_M_capteurs(m,1) = exp(-i*pi*(m-1)*sin(teta)); %on calcul les vecteur directionnels pour chaque teta
    end
